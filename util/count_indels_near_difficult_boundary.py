#!/usr/bin/env python3
"""
Count Team B indel calls that land immediately upstream of difficult regions.

For each ALT allele (default: somatic) in the provided Team B parquet, the script
checks whether the indel position lies 1–10bp before the start of any interval
in the GRCh38 difficult-region union BED. It reports the counts for each offset
bucket and prints summary totals.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Count Team B indels within 10bp upstream of difficult-region starts."
    )
    parser.add_argument(
        "--team-parquet",
        type=Path,
    ...
    parser.add_argument(
        ...
        help=f"Maximum distance (bp) upstream to analyze (default: {DEFAULT_WINDOW}).",
    )
    parser.add_argument(
...
def count_offsets(
    ...
    """
    Count how many alt alleles fall within each offset (1-10bp) upstream of
    a difficult-region start.
    """
    ...
    for chrom, chrom_df in alt_df.groupby("chrom"):
        ...
        # For each position, find the first interval start greater than pos0
        idx_right = np.searchsorted(starts0, pos0, side="right")

        for pos_1b, pos0_val, idx in zip(positions, pos0, idx_right):
            total_evaluated += 1

            # Skip if inside a difficult region (previous interval still covers pos)
            if idx > 0 and pos0_val < ends0[idx - 1]:
                continue

            if idx == starts0.size:
                continue  # No interval start after this position

            start0 = starts0[idx]
            diff = (start0 + 1) - pos_1b  # convert start to 1-based before diff
            if diff in counts:
                counts[diff] += 1
...
def main() -> None:
    ...
    print("\nCounts of ALT alleles upstream of difficult-region starts")
    ...
    print(f"Total within 1-{args.window}bp of a difficult-region start: {total_near}")
    ...
    if plot_path:
        ...
        distances = summary_df["distance_bp_from_start"]
        ax.bar(distances, summary_df["count"], color="tab:orange")
        ax.set_xlabel("Distance from difficult-region start (bp)")
        ax.set_ylabel("Indel count")
        ax.set_title("Somatic ALT indels upstream of difficult-region starts (in not difficult)")
